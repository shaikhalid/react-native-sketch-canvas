  ...
  componentWillMount() {
    this.panResponder = PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        if (!this.props.touchEnabled) return
        if (this._path) {
          const x = parseFloat((gestureState.x0 + (gestureState.dx)/(this.props.zoomLevel*this.props.zoomLevel) - this._offset.x).toFixed(2)),
                y = parseFloat((gestureState.y0 + (gestureState.dy)/(this.props.zoomLevel*this.props.zoomLevel) - this._offset.y).toFixed(2))
